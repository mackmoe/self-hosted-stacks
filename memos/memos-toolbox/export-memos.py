...
def export_memo(memo: Dict[str, Any], output_dir: str, include_metadata: bool = True) -> Optional[str]:
    ...
    if include_metadata:
        export_content += f"<!-- Exported from Memos -->\n"
        export_content += f"<!-- Memo ID: {memo_id} -->\n"
        if timestamp:
            export_content += f"<!-- Created: {timestamp} -->\n"
        if memo.get("visibility"):
            export_content += f"<!-- Visibility: {memo.get('visibility')} -->\n"
        export_content += f"\n"
